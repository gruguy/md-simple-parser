<template>
  <div class="wrapper">
    <div class="item">
      <span class="btn ml20" @click="md_source_render_copy">复制源代码</span>
...
export default {
  ...
  data(){
    return {
      ...
      selectedCss: '',
      isSelectLineNum: false
    }
  },
  
  mounted(){
    
    this.isSelectLineNum = localStorage.getItem('$showLineNumbers');
  ...
  computed: {
    ...mapGetters(['editorContent', 'intimeMode'])
  },
  methods: {
    ...mapMutations({
    ...
    showLineNum(){
      let blocks = document.querySelectorAll("pre code");
      this.isSelectLineNum = !this.isSelectLineNum;
      localStorage.setItem('$showLineNumbers', this.isSelectLineNum);
      if(this.isSelectLineNum) {
        blocks.forEach(block => {
        // hljs.highlightBlock(block);
